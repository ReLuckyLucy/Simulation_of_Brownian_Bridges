...
# 初始点和终止点
ax.scatter(np.zeros(N), x0[:, 0], x0[:, 1], color='red', label='初始点 (t=0)')
ax.scatter(np.ones(N) * T, xf[:, 0], xf[:, 1], color='black', label='终止点 (t=T)')

# 画布朗桥路径
for i in range(N):
    ax.plot(t, x_bb[i, :, 0], x_bb[i, :, 1], alpha=0.3)
