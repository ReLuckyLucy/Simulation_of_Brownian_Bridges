...
def generate_moon(N, noise=0.1, radius=5, center_up=(0,0,0), center_down=(0,0,0)):
    ...
    return np.hstack((np.vstack((x_s1, y_s1, z_s1)), np.vstack((x_s2, y_s2, z_s2)))).T
...
# 初始点和终止点
ax.scatter(t[0] * np.ones(N), x0[:, 0], x0[:, 1], color='red', label='初始点 (t=0)', alpha=0.6)
ax.scatter(t[-1] * np.ones(N), xf[:, 0], xf[:, 1], color='black', label='终止点 (t=T)', s=10, alpha=0.6)
